...
- **背景音乐** — 右下角音乐按钮：优先播放 `assets/audio/bgm.mp3`（放入你已授权的
  音源即可）；无文件时回退到内置**原创**吉卜力风温柔民谣（Web Audio 实时合成：
  76 BPM 行板、竖琴分解和弦、带颤音的吟唱主旋律、主歌+副歌 16 小节循环）。
  偏好自动记忆，回访后首次点击页面即恢复播放。
